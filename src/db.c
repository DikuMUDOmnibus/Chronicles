...
void boot_db( bool fCopyOver )
{
    ...
    /* default values */
    sysdata.read_all_mail		= LEVEL_HIGH_ADMIN;
    sysdata.read_mail_free 		= LEVEL_IMMORTAL;
    sysdata.write_mail_free 		= LEVEL_IMMORTAL;
    sysdata.take_others_mail		= LEVEL_HIGH_ADMIN;
    sysdata.muse_level			= LEVEL_WORKER;
    sysdata.think_level			= LEVEL_MASTER;
    sysdata.build_level			= LEVEL_WORKER;
    sysdata.log_level			= LEVEL_LOG;
    sysdata.level_modify_proto		= LEVEL_WORKER;
    sysdata.level_override_private	= LEVEL_WORKER;
    sysdata.level_mset_player		= LEVEL_WORKER;
    ...
    sysdata.level_getobjnotake 		= LEVEL_WORKER;
    ...
    sysdata.wiz_lock			= 0;
    sysdata.newbie_stats[0]		= 20;
    sysdata.newbie_stats[1]		= 100;
    sysdata.newbie_stats[2]		= 100;
    ...
    log_string( "Loading qdata configuration..." );

    qdata.minprac_gain		= 1;
    qdata.maxprac_gain		= 6;
    qdata.mingold_gain		= 10000;
    qdata.maxgold_gain		= 60000;
    qdata.minqp_gain		= 10;
    qdata.maxqp_gain		= 60;
    qdata.minqt_comp		= 10;
    qdata.maxqt_comp		= 25;
    ...
    qdata.maxq_time		= 15;
    qdata.qp_loss		= 20;
    qdata.qtime_next_quit	= 30;
    qdata.qtime_next_fail	= 25;
    qdata.qtime_next_clear	= 5;
    qdata.qlevel_diff		= 15;
    qdata.quest_chance		= 75;
    qdata.object_chance		= 60;
    qdata.give_quest		= -1;
    qdata.prac_chance		= 10;
...
void load_area( FILE *fp )
{
    ...
    pArea->derivatives  = STRALLOC( "" );
    pArea->room_name_format = NULL;
    pArea->room_desc_format = NULL;
...
void load_derivatives( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_derivatives: no #AREA seen yet." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
...
/*
 * Load a name format section. -Orion
 */
void load_name_format( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_name_format: no #AREA seen yet." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
    ...
    tarea->room_name_format = fread_string( fp );
    return;
}
 ...
 * Load a name format section. -Orion
 */
void load_desc_format( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_desc_format: no #AREA seen yet." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
    ...
    tarea->room_desc_format = fread_string( fp );
    return;
}
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->max_hit		= generate_hp( mob->level, pMobIndex->hitnodice, pMobIndex->hitsizedice, pMobIndex->hitplus );
    mob->hit			= mob->max_hit;
...
void clear_char( CHAR_DATA *ch )
{
    ...
    ch->practice		= 0;
    ch->max_hit			= ch->hit		= sysdata.newbie_stats[0];
    ch->max_mana		= ch->mana		= sysdata.newbie_stats[1];
    ch->max_move		= ch->move		= sysdata.newbie_stats[2];
    ch->height			= 72;
    ch->weight			= 180;
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&wRooms:   &W%5d\t\t\t\n\r",
        top_room   );
...
/*  towizfile( " Masters of the Realms of Despair!" );*/
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
	case MAX_LEVEL		: towizfile( " Supreme Entity" );	break;
	case LEVEL_HIGH_ADMIN	: towizfile( " High Admin" );		break;
	case LEVEL_MASTER	: towizfile( " Masters" );		break;
	case LEVEL_WORKER	: towizfile( " Workers" );		break;
	case LEVEL_SLAVE	: towizfile( " Slaves" );		break;
	case LEVEL_SLAVE - 1	: towizfile( " Retired" );		break;
	case LEVEL_SLAVE - 2	: towizfile( " Guests" );		break;
...
void load_area_file( AREA_DATA *tarea, char *filename )
{
	...
	else if ( !str_cmp( word, "DERIVATIVES") ) load_derivatives(tarea, fpArea);
        else if ( !str_cmp( word, "NAMEFORMAT"  ) ) load_name_format(tarea, fpArea);
        else if ( !str_cmp( word, "DESCFORMAT"  ) ) load_desc_format(tarea, fpArea);
...
void save_sysdata( SYSTEM_DATA sys )
{
	...
	fprintf( fp, "NewbieStats    %d %d %d\n", sys.newbie_stats[0], sys.newbie_stats[1], sys.newbie_stats[2]	 );
...
void fread_sysdata( SYSTEM_DATA *sys, FILE *fp )
{
	...
	case 'N':
		...
		sys->newbie_stats[0] = x1;
		sys->newbie_stats[1] = x2;
		sys->newbie_stats[2] = x3;
                fMatch = TRUE;
		break;
	    }
	    break;
