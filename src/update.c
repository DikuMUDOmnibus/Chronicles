...
void gain_exp( CHAR_DATA *ch, int gain )
{
    ...
    if ( ( IS_AVATAR( ch ) || IS_DEMIGOD( ch ) ) && !IS_IMMORTAL( ch ) )
    {
	DESCRIPTOR_DATA *d;
	char message[MAX_STRING_LENGTH];

	sprintf( message, "%s", IS_DEMIGOD( ch ) ? "has risen in power and become a Demi-God!" : "achieved the next level of Avatarhood!" );
	sprintf( buf, "%s has just %s!", ch->name, message );
	...
	set_char_color( AT_WHITE, ch );
	if ( !IS_DEMIGOD( ch ) )
	    do_help( ch, "M_ADVDEMIGOD_" );
	else
	    do_help( ch, "M_ADVAVATAR_" );
    }
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	{
	case COND_FULL:
          if ( !IS_AVATAR( ch ) && ch->class != CLASS_VAMPIRE )
          {
...
{
    /*
     *  Group alignment/class restrictions. -Orion Elder
     */
    if ( ch->pcdata && ch->pcdata->faction )
    {
	if ( check_factionflags( ch, ch->pcdata->faction ) == TRUE )
        {
	    send_to_char_color( "&G&rThe Gods' Avatar tells you '&G&zYou are not following the ideals of your group! SUFFER!&G&r'\n\r", ch );
...
{   
	...
	if ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL )
	{
           ...
           ch->pcdata->release_date <= current_time )
      {
         ...
         if ( ch->pcdata->faction )
          location = get_room_index( ch->pcdata->faction->recall );
         else
	...
	if ( !char_died(ch) )
	{
	    ...
	    if ( ch == ch_save && xIS_SET( sysdata.save_flags, SV_AUTO )
