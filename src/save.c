...
void save_char_obj( CHAR_DATA *ch )
{
    ...
    if ( xIS_SET( sysdata.save_flags, SV_BACKUP ) || 
       ( xIS_SET( sysdata.save_flags, SV_QUITBACKUP ) && quitting_char == ch ))
    {
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	if ( ch->pcdata->faction_name && ch->pcdata->faction_name[0] != '\0' )
	  fprintf( fp, "Faction      %s~\n",	ch->pcdata->faction_name );
        fprintf( fp, "Flags        %d\n",	ch->pcdata->flags	);
	...
	if ( ch->desc && ch->desc->host )
        ...
        {
	    fprintf( fp, "QTN          %d\n",  ch->pcdata->quest_timer[QTIMER_NEXT]	);
        }
        else if ( ch->pcdata->quest_timer[QTIMER_CURR] > 0 )
        {
    ...
    track = URANGE( 2, ((ch->level+3) * MAX_KILLTRACK)/LEVEL_DEMIGOD, MAX_KILLTRACK );
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name, bool preload, bool copyover )
{
    ...
    ch->pcdata->quest_timer[QTIMER_NEXT]= 0;
	...
	log_string_plus( buf, LOG_COMM, LEVEL_MASTER );
      }
    }
    /* else no player file */
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload, bool copyover )
{
	...
	/* 'E' was moved to after 'S' */
	    ...
	    {
		ch->pcdata->faction_name = fread_string( fp );

		if ( !preload
		&&   ch->pcdata->faction_name[0] != '\0'
		&& ( ch->pcdata->faction = get_faction( ch->pcdata->faction_name )) == NULL )
		{
	...
	case 'Q':
	    KEY( "QTN",   ch->pcdata->quest_timer[QTIMER_NEXT],	fread_number( fp ));
	    break;
	...
	case 'E':
		...
		if (!ch->pcdata->prompt )
		  ...
		  ch->pcdata->fprompt   = STRALLOC( "" );
		ch->editor		= NULL;
		killcnt = URANGE( 2, ((ch->level+3) * MAX_KILLTRACK)/LEVEL_DEMIGOD, MAX_KILLTRACK );
		if ( killcnt < MAX_KILLTRACK )
...
CHAR_DATA *  fread_mobile( FILE *fp )
{
  ...
  if ( !strcmp(word, "Vnum") )
  {
    ...
    if ( !pMobIndex )
    {
	bug("Fread_mobile: Can not get_mob_index for vnum %d", vnum );
	return NULL;
    }
