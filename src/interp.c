...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	for ( cmd = command_hash[LOWER(command[0])%126]; cmd; cmd = cmd->next )
	{
	    ...
	    /*
	     * Command name checking inspired by Eos of AuricMUD. -Orion
	     */
	    fCommand = IS_SET( cmd->flags, CMD_WHOLE_NAME ) ? !str_cmp( command, cmd->name ) : !str_prefix( command, cmd->name );
...
check_cmd_flags ( CHAR_DATA *ch, CMDTYPE *cmd )
{
    ...
    if ( !IS_NPC( ch ) && IS_IMMORTAL( ch ) && ( get_trust( ch ) < LEVEL_HIGH_ADMIN ) )
    {
	if ( IS_SET( cmd->flags, CMD_ADMIN	) || IS_SET( cmd->flags, CMD_ENFORCER ) ||
	     IS_SET( cmd->flags, CMD_BUILDER	) || IS_SET( cmd->flags, CMD_ROLEPLAYER ) )
	{
	    bool fUseable = FALSE;
	    ...
	    if ( IS_SET( cmd->flags, CMD_ROLEPLAYER ) && IS_SET( ch->pcdata->flags, PCFLAG_ROLEPLAYER ) )
		fUseable = TRUE;

	    if ( fUseable == FALSE )
	    {
		sprintf( cmd_flag_buf, "%s is not a command allowed to you by your current status.\n\r", cmd->name );
	    }
	    else
		cmd_flag_buf[0] = '\0';
	}
