...
bool can_read( CHAR_DATA *ch, BOARD_DATA *board )
{
  ...
  {
    if ( ch->pcdata->faction && !str_cmp( ch->pcdata->faction->name, board->read_group ) ) 
      return TRUE; 
...
bool can_post( CHAR_DATA *ch, BOARD_DATA *board )
{
  ...
  {
    if ( ch->pcdata->faction && !str_cmp( ch->pcdata->faction->name, board->post_group ) ) 
      return TRUE; 
...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
	...
	if ( !can_read( ch, board ) )
	{
		...
		if ( (first_list && vnum >= first_list) || !first_list )
		   pager_printf_color( ch, "&C%2d&c%c &C%-12s&c%c &C%-24s &c: &C%-35s\n\r",
			vnum,
			is_note_to( ch, pnote ) ? ']' : '>',
			pnote->sender,
	...
	else
	{
		...
		if (is_note_to( ch, pnote ) || get_trust(ch) >= sysdata.read_all_mail)
          	    ch_printf_color( ch, "&C%2d&c%c &C%-12s&c: &C%-13s &c: &C%s\n\r",
			++vnum,
			is_note_to( ch, pnote ) ? '-' : '}',
			pnote->sender,
			pnote->subject );
	...
	if ( !can_read( ch, board ) )
	{
		...
		vnum++;
		if ( (first_list && vnum >= first_list) || !first_list )
		   pager_printf_color( ch, "&C%2d&c%c &C%-12s&c%c &C%-12.12s &c: &C%s\n\r",
			vnum,
			is_note_to( ch, pnote ) ? ']' : '>',
			pnote->sender,
			(pnote->voting != VOTE_NONE) ? (pnote->voting == VOTE_OPEN ? 'V' : 'C') : ':',
	...
	else
	{
            ...
            for ( pnote = board->first_note; pnote; pnote = pnote->next )
		if (is_note_to( ch, pnote ) || get_trust(ch) >= sysdata.read_all_mail)
          	    ch_printf_color( ch, "&C%2d&c%c &C%s&c: &C%s\n\r",
			++vnum,
			is_note_to( ch, pnote ) ? '-' : '}',
			pnote->sender,
			pnote->subject );
