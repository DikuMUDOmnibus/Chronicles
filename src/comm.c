...
int init_socket( int port )
{
    ...
    if ( listen( fd, MAX_CONNECT_SOCKET ) < 0 )
    {
...
bool write_to_descriptor( int desc, char *txt, int length )
{
	...
	/*
	 * If nWrite is negative one, then that means that send() errored out. -Orion
	 */
	if ( nWrite == -1 )
	{
#ifdef WIN32
	    ...
	    /*
	     * If iErr is EWOULDBLOCK then that means the socket is blocking, and we
	     * need to handle it... otherwise something is screwed, so error and then
	     * return FALSE -Orion
	     */
	    if ( iErr == EWOULDBLOCK )
	    {
		/*
		 * This is a SPAMMY little bugger. If you don't NEED this, leave it
		 * commented out. -Orion.
		 *
...
void show_title( DESCRIPTOR_DATA *d )
{
    ...
    d->connected = CON_PRESS_ENTER_MENU;
}
...
/*	extern int lang_array[];
	...
	{
	case 'y':
	case 'Y': xSET_BIT(ch->act,PLR_ANSI);  break;
	case 'n':
	case 'N': break;
	default:
	...
	case '4':
	    send_to_pager( "\n\rCurrently working on to fix security issues.\n\r\n\r", ch);
	    do_help( ch, "login_menu" );
	    return;
	    break;
	...
	if ( IS_IMMORTAL(ch) )
	  ...
	  do_help( ch, "hmotd" );
	else if ( !IS_HERO( ch ) && ch->level > 0 )
	  do_help( ch, "motd" );
	else if ( ch->level == 0 )
	  do_help( ch, "nmotd" );
	send_to_pager( "\n\rPress [ENTER] ", ch );
	if ( ch->level == 0 || xIS_SET( ch->act, PLR_AUTONEWS ) )
	    d->connected = CON_READ_NEWS;
	else
...
void write_to_pager( DESCRIPTOR_DATA *d, const char *txt, int length )
{
  ...
  {
    if ( d->pagesize > MAX_STRING_LENGTH*32 ) //128,000
    {
...
char *default_prompt( CHAR_DATA *ch )
{
    ...
    sprintf( buf, "&r&Y&G&w%s&G&Z::", !IS_NPC( ch ) ? ch->name : ch->short_descr );
    strcat( buf, "&G&W[ &G&Y%h&G&W/&G&Y%H&G&Chp " );
    ...
    strcat( buf, " &Y&G%v&G&W/&Y&G%V&G&Cmv " );

    if ( !IS_NPC( ch ) && !IS_IMMORTAL( ch ) )
	strcat( buf, "&G&w%X&G&Cexp " );
...
void display_prompt( DESCRIPTOR_DATA *d )
{
    ...
    const char *prompt;
    const char *helpstart = "&r&Y&G&c[&G&CType HELP START&G&c] &G&W";
    char buf[MAX_STRING_LENGTH];
    char *pbuf = buf;
    int stat, percent;
		...
		switch(*prompt)
		{
			...
			*pbuf = '\0';
			...
			if ( !IS_QUESTING( ch ) )
			{
				...
				stat = ch->pcdata->quest_timer[QTIMER_NEXT];
			    else
				strcpy( pbuf, "None" );
			}
			else
			{
			    if ( ch->pcdata->quest_timer[QTIMER_CURR] > 0 )
				stat = ch->pcdata->quest_timer[QTIMER_CURR];
			    else
				strcpy( pbuf, "Unlimited" );
			}
			break;
		    case 'r':
			if ( IS_IMMORTAL( och ) )
			...
			break;
		    case 'R':
			if ( IS_IMMORTAL( och ) && xIS_SET( och->act, PLR_ROOMVNUM ) )
