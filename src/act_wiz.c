...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
	...
	send_to_pager( "\n\r\n\r", ch );
	/*
	 * Adjust col to include the '[LEVEL XXX]' as a piece of the command
	 * list. -Orion
	 */
	col = 1;
	pager_printf_color( ch, "&r&Y&G&c[&G&CLEVEL %-3d&G&c]&G&w ", level );
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
     {
        ...
        to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_SLAVE );
...
void do_transfer( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg1, "all" ) && get_trust(ch) >= LEVEL_MASTER) 
    {
	for ( d = first_descriptor; d; d = d->next )
	{
...
void do_at( CHAR_DATA *ch, char *argument )
{
    ...
    {
      if ( get_trust( ch ) < LEVEL_HIGH_ADMIN )
      {
	send_to_char( "That room is private right now.\n\r", ch );
	return;
      }
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	for ( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next ) 
	    ch_printf_color( ch,
		"&W%2d)  &w%s to %-5d  &cKey: &w%d  &cFlags: &w%s\n\r  &cKeywords: '&w%s&c'\n\r     Exdesc: &w%s     &cBack link: &w%d  &cVnum: &w%d  &cPulltype: &w%s  &cPull: &w%d\n\r",
		++cnt,
		dir_text[pexit->vdir],
    ...
    for ( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next )
	ch_printf( ch,
	  "%2d) %-2s to %-5d.  Key: %d  Keywords: %s  Flags: %s.\n\r",
		++cnt,
		dir_text[pexit->vdir],
		pexit->to_room ? pexit->to_room->vnum : 0,
		pexit->key,
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim) && get_trust( ch ) < LEVEL_HIGH_ADMIN
	...
	pager_printf_color( ch, "   &c%s: &w%s",
	    victim->pcdata->faction->faction_type == FACTION_ORDER ? "Order" :
	    victim->pcdata->faction->faction_type == FACTION_GUILD ? "Guild" : "Clan",
	    victim->pcdata->faction->name );
    send_to_pager( "\n\r", ch );
    if ( get_trust(ch) >= LEVEL_WORKER && !IS_NPC(victim) && victim->desc )
    ...
    if ( IS_BLOODSUCKER(victim) && !IS_NPC(victim) )
    {
...
#ifdef TOOSNOOPY
    if ( get_trust(victim) > LEVEL_WORKER && get_trust(ch) < LEVEL_SUPREME )
...
void do_statshield( CHAR_DATA *ch, char *argument )
{
    ...
    one_argument( argument, arg );
    if ( IS_NPC ( ch ) || get_trust( ch ) < LEVEL_HIGH_ADMIN )
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_switch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(victim) && ch->level < LEVEL_MASTER )
    {
...
void do_immortalize( CHAR_DATA *ch, char *argument )
 {
    ...
    if ( victim->pcdata->faction )
    {
        ...
        --victim->pcdata->faction->members;
        if (!str_cmp (victim->name, victim->pcdata->faction->leader) )
        {
           STRFREE (victim->pcdata->faction->leader);
           victim->pcdata->faction->leader = STRALLOC ("");
        }
        if (!str_cmp (victim->name, victim->pcdata->faction->number1))
        {
           STRFREE (victim->pcdata->faction->number1);
           victim->pcdata->faction->number1 = STRALLOC ("");
        }
        if (!str_cmp (victim->name, victim->pcdata->faction->number2))
        {
           STRFREE (victim->pcdata->faction->number2);
           victim->pcdata->faction->number2 = STRALLOC ("");
        }
...
void do_restore( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg2 );
    if ( !str_cmp( arg2, "boost" ) && get_trust( ch ) >= LEVEL_MASTER )
    {
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
		...
		vch->pcdata->condition[COND_BLOODTHIRST] = GetMaxBlood( vch );
		update_pos (vch);
    ...
    victim->move = victim->max_move;
    if ( victim->pcdata )
      victim->pcdata->condition[COND_BLOODTHIRST] = GetMaxBlood( victim );
    update_pos( victim );
    if ( ch != victim )
...
void do_force( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( ch ) < LEVEL_WORKER && IS_NPC(victim)
...
void do_rassign( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) || get_trust( victim ) < LEVEL_SLAVE )
    {
...
{
    char pass[MAX_STRING_LENGTH], name[MAX_STRING_LENGTH];
...
void close_area( AREA_DATA *pArea )
{
  ...
  if (pArea->derivatives);
    STRFREE(pArea->derivatives);
  if (pArea->room_name_format);
    STRFREE(pArea->room_name_format);
  if (pArea->room_desc_format);
    STRFREE(pArea->room_desc_format);
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
    ...
    /*
     * Check to see if the victim is online. -Orion
     */
    for ( victim = first_char; victim; victim = victim->next )
    {
	/*
	 * Is the person being destroyed online? If so, break out. -Orion
	 */
	...
	{
	    ch_printf( ch, "Unknown error #%d - %s (immortal data). Report immediately.\n\r", errno, strerror( errno ) );
	    sprintf( buf2, "%s destroying %s", ch->name, buf );
	    perror( buf2 );
	}
	...
	for ( pArea = first_build; pArea; pArea = pArea->next )
	{
		...
		{
		    ch_printf( ch, "Unknown error #%d - %s (area data). Report immediately.\n\r", errno, strerror( errno ) );
		    sprintf( buf2, "%s destroying %s", ch->name, buf );
		    perror( buf2 );
		}
		break;
    ...
    {
	set_char_color( AT_WHITE, ch );
	ch_printf( ch, "Unknown error #%d - %s. Report immediately.\n\r", errno, strerror( errno ) );
...
void do_cset( CHAR_DATA *ch, char *argument )
{
  ...
  if (argument[0] == '\0')
  { 
    pager_printf_color(ch, "\n\r&WMud_name: &w%s", sysdata.mud_name );
    pager_printf_color(ch, "\n\r&WStarting Stats:\n\r  &wHitpoints: &W%d  &wMana: &W%d  &wMove: &W%d",
	    sysdata.newbie_stats[0], sysdata.newbie_stats[1], sysdata.newbie_stats[2] );
    ...
    pager_printf_color(ch, "&WOther:\n\r  &wForce on players:             &W%-7d", sysdata.level_forcepc);
    pager_printf_color(ch, "&wPrivate room override:         &W%-3d\n\r", sysdata.level_override_private);
    ...
    pager_printf_color(ch, "&wRequired who argument:         &W%-3s\n\r", (sysdata.reqwho_arg) ? "ON" : "off");
    pager_printf_color(ch, "  &wSave flags: &W%s\n\r", ext_flag_string( &sysdata.save_flags, save_flag ) );
    pager_printf_color(ch, "  &wIdents retries:               &W%-7d", sysdata.ident_retries);
    pager_printf_color(ch, "&wWizlock is:                    &W%-7s\n\r", (sysdata.wiz_lock) ? "ON" : "off" );
    pager_printf_color(ch, "  &wAutoboot Time:                &W%2.2d:%-4.2d", sysdata.autoboot_hour, sysdata.autoboot_minute);
    pager_printf_color(ch, "&wAutoboot Period:               &W%-7d\n\r", sysdata.autoboot_period);
    return;
  }
  ...
  smash_tilde( argument );
	...
	if ( !arg2 || arg2[0] == '\0' )
	    do_help( ch, "cset" );
	else if ( !str_cmp( arg2, "saveflags" ) || !str_cmp( arg2, "saveflag" ) )
	    do_help( ch, "saveflags" );
	else
	    do_cset( ch, "help" );	
  ...
  if (!str_prefix( arg, "autoboot_time" ) )
  {
    ...
    hour = (sh_int) atoi( arg1 );
    minute = (sh_int) atoi( arg2 );
  ...
  if (!str_prefix( arg, "start_hp" ) )
  {
    sysdata.newbie_stats[0] = level;
    send_to_char("Ok.\n\r", ch);
    return;
  }

  if (!str_prefix( arg, "start_mana" ) )
  {
    sysdata.newbie_stats[1] = level;
    send_to_char("Ok.\n\r", ch);
    return;
  }

  if (!str_prefix( arg, "start_move" ) )
  {
    sysdata.newbie_stats[2] = level;
    send_to_char("Ok.\n\r", ch);
    return;
  }
...
void do_unhell( CHAR_DATA *ch, char *argument )
{
  ...
  if ( victim->pcdata->faction )
    location = get_room_index(victim->pcdata->faction->recall);
  else
...
void do_sedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust(ch) > LEVEL_WORKER && !str_cmp( arg1, "save" ) )
    {
	save_socials();
	send_to_char( "Saved.\n\r", ch );
	return;
    ...
    if ( get_trust(ch) > LEVEL_WORKER && !str_cmp( arg2, "delete" ) )
    {
    ...
    if ( get_trust(ch) > LEVEL_MASTER && !str_cmp( arg2, "name" ) )
    {
	bool relocate;
...
void add_command( CMDTYPE *command )
{
    ...
    if ( !command->code )
    {
	bug( "%s", "Add_command: NULL command->code" );
	return;
    }
...
void do_cedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust(ch) > LEVEL_MASTER &&    !str_cmp( arg2, "create" ) )
    {
	if ( command )
	{
	...
	if ( command->do_fun == skill_notfound )
	{
	    ...
	    command->code = str_dup( "skill_notfound" );
	}
	else
	    command->code = str_dup( arg2 );
	add_command( command );
	...
	ch_printf( ch, "Command:  %s\n\rLevel:    %d\n\rPosition: %d\n\rLog:      %d\n\rCode:     %s\n\rFlags:  %s\n\r",
	    ...
	    command->code, flag_string(command->flags, cmd_flags));
...
void do_showclass( CHAR_DATA *ch, char *argument )
{
    ...
    pager_printf_color( ch, "&wCLASS: &W%s\n\r&wPrime Attribute: &W%-14s  &wWeapon: &W%-5d\n\r",
	class->who_name, affect_loc_name(class->attr_prime), class->weapon );
...
void do_setrace( CHAR_DATA *ch, char *argument )
{
	...
	if ( is_number( argument ) )
	{
	    argVal = atoi( argument );
	    isNum = TRUE;
	}

	for ( langVal = 0; lang_array[langVal] != LANG_UNKNOWN; langVal++ )
	{
	    if ( ( isNum == TRUE && ( langVal == argVal ) ) || !str_cmp( argument, lang_names[langVal] ) )
	    {
		race->language = lang_array[langVal];
		foundLang = TRUE;
		break;
            }
	}
	...
	if ( !foundLang )
	    send_to_char( "Language not found.\n\r", ch );
	else
	    ch_printf( ch, "%s language set to %s.\n\r", race->race_name, lang_names[langVal] );
...
void do_showrace( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( race->language == lang_array[i] )
	{
	    ch_printf( ch, "Language: %s\n\r\n\r", lang_names[i] );
	    break;
	}
    }
   ...
   if( !str_cmp( arg1, "log" ) )
   {
	...
	return;
      ...
      if( !str_cmp( arg2, "list" ) )
      {
	    ...
	    str_cmp(ch->name, pproject->owner) && get_trust(ch) < LEVEL_SLAVE 
 ...
 * by Gorog. It's only used in the three functions below, but as I mentioned with the wtrunc 
...
/*
 * Search objects for obj progs containing a specified text string. This is from the grub.c
 * file. I eliminated that file, but this code could, eventually, prove useful. -Orion
 */
...
/*
 * Search mobs for mob progs containing a specified text string. This is from the grub.c
 * file. I eliminated that file, but this code could, eventually, prove useful. -Orion
 */
...
/*
 * Search rooms for room progs containing a specified text string. This is from the grub.c 
 * file. I eliminated that file, but this code could, eventually, prove useful. -Orion
 */
