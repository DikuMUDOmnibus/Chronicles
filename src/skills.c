...
bool is_legal_kill(CHAR_DATA *ch, CHAR_DATA *vch)
{
  ...
  if ( !IS_PKILL(ch) || !IS_PKILL(vch) )
    return FALSE;
  if ( ch->pcdata->faction && ch->pcdata->faction == vch->pcdata->faction )
    return FALSE;
  return TRUE;
}
...
void do_slookup( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Flags: %s  Value: %d  Info: %d  Code: %s\n\r",
...
void do_sset( CHAR_DATA *ch, char *argument )
{
	...
	skill->code = str_dup( "spell_smaug" );
	skill->type = type;
	...
	if ( !str_cmp( arg2, "flag" ) )
	{
	    ...
	    while( argument && argument[0] != '\0' && (argument = one_argument( argument, buf )) )
	    {
		x = get_sflag( buf );
        ...
        if ( can_use_skill( ch, number_percent(), gsn_feed ) )
	{
	  ...
	  &&  ch->pcdata->condition[COND_BLOODTHIRST] < GetMaxBlood( ch ) )
	  {
	    gain_condition( ch, COND_BLOODTHIRST,
	     ...
	     GetMaxBlood( ch ) - ch->pcdata->condition[COND_BLOODTHIRST] ) );
...
void do_mistwalk( CHAR_DATA *ch, char *argument )
{
    ...
    ||  (IS_SET(victim->in_room->area->flags, AFLAG_ANTIPKILL) && IS_PKILL(ch)))
    {
        send_to_char( "You are unable to sense your victim.\n\r", ch );
...
void do_scan( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level < LEVEL_DEMIGOD ) --max_dist;
    if ( ch->level < ((LEVEL_DEMIGOD/5) * 4) ) --max_dist;
    if ( ch->level < ((LEVEL_DEMIGOD/5) * 3) ) --max_dist;
...
CHAR_DATA *scan_for_victim( CHAR_DATA *ch, EXIT_DATA *pexit, char *name )
{
    ...
    was_in_room = ch->in_room;
    if ( IS_BLOODSUCKER(ch) && time_info.hour < 21 && time_info.hour > 5 )
	max_dist = 1;
