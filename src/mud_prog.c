...
doneargs:
    ...
    *p = '\0';
	...
	if ( !str_cmp(chck, "faction") )
	{
	    if ( IS_NPC(chkchar) || !chkchar->pcdata->faction )
		return FALSE;
	    return mprog_seval(chkchar->pcdata->faction->name, opr, rval, mob);
	}
	/* Check added to see if the person isleader of == faction Shaddai */
	if (!str_cmp (chck, "isleader"))
	{
	    FACTION_DATA *temp;
	    if ( IS_NPC ( chkchar ) )
		return FALSE;
	    if ( (temp = get_faction( rval )) == NULL )
		return FALSE;
...
*/
	...
	/* Check added to see if the person isleader of == faction Gorog */
	if (!str_cmp (chck, "isfactionleader"))
	{
	    FACTION_DATA *temp;
	    if ( IS_NPC ( chkchar ) )
		return FALSE;
	...
	if ( !str_cmp(chck, "factiontype") )
	{
	    if ( IS_NPC(chkchar) || !chkchar->pcdata->faction )
		return FALSE;
	    return mprog_veval(chkchar->pcdata->faction->faction_type, opr, atoi(rval), mob);
	}
