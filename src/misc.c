...
void do_drink( CHAR_DATA *ch, char *argument )
{
    ...
    case ITEM_BLOOD:
	if ( IS_BLOODSUCKER(ch) && !IS_NPC(ch) )
	{
	    ...
	    if ( ch->pcdata->condition[COND_BLOODTHIRST] < GetMaxBlood( ch ) )
	    {
...
void pullorpush( CHAR_DATA *ch, OBJ_DATA *obj, bool pull )
{
	...
	if ( pSlots )
	{
	    sprintf( buf, "$n %s $p lever.", pull ? "pulls" : "pushes" );
	}
	else
	{
	...
	if ( pSlots )
	{
	    sprintf( buf, "You %s $p lever.", pull ? "pull" : "push" );
	}
	else
	{
...
void do_smoke( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_VALID_HERB( pipe->value[2] ) && pipe->value[2] < top_herb )
	{
	    ...
	    if ( skill->spell_fun )
		obj_cast_spell( sn, UMIN(pipe->level, ch->level), ch, ch, NULL );
	...
	case ITEM_KEYRING:
	  ...
	  else
	  {
		...
		if ( empty_obj( obj, dest, NULL ) )
		{
		    ...
		    if ( !dest->carried_by
		    &&    xIS_SET( sysdata.save_flags, SV_EMPTY ) )
			save_char_obj( ch );
		}
		else
 ...
 */
bool fptof( FILE *stream, char *data )
{
...
/*
 * Find the search value in string and replace it with the replace value.
 * Can handle case sensitive search/replace as well. -Orion
 */
char *str_replace( char *search, char *replace, char *body, bool cs )
{
    int slen = 0, rlen = 0, blen;
    bool removal = FALSE;
    static char new_string[MAX_STRING_LENGTH*2];

    /*
     * If there is no body string, we have nothing to do, return NULL. -Orion
     */
    if ( !body || body[0] == '\0' )
    {
	return NULL;
    ...
    /*
     * If there is no search string, we have nothing to replace, else grab the
     * length of the search string. -Orion
     */
    if ( !search || search[0] == '\0' )
    {
	sprintf( new_string, "%s%c", body, '\0' );
	return new_string;
    }
    else
	slen = strlen( search );
    ...
    /*
     * If there is no replace string, we will be removing the search string
     * entirely, else grab the length of the replace. -Orion
     */
    ...
    /*
     * If the case comparison matches, and we're doing a case-sensitive s&r then
     * there is no need to continue, else we do the s&r. -Orion
     */
    if ( cs == TRUE && !strcmp( search, replace ) )
    {
	sprintf( new_string, "%s%c", body, '\0' );
	return new_string;
    }
	...
	{
	    if ( ( cs == TRUE && ( body[i] == search[0] ) ) ||
	         ( cs == FALSE && ( tolower( body[i] ) == tolower( search[0] ) ) ) )
	    {
		int i2, j2;
		bool sMatch = FALSE;

		for( i2=i+1, j2=1; j2 < slen; i2++, j2++ )
		{
		    bool cMatch = FALSE;

		    if ( ( cs == TRUE && ( body[i2] == search[j2] ) ) ||
		         ( cs == FALSE && ( tolower( body[i2] ) == tolower( search[j2] ) ) ) )
			cMatch = TRUE;
		...
		{
		    /*
		     * If removal is TRUE we do nothing. -Orion
		     */
		    if ( removal == FALSE )
		    {
			for( i2=0; i2 < rlen; i2++ )
			{
			    new_string[j] = replace[i2];
			    j++;
			}
			j--;
		    }
