...
{
  if ( IS_BLOODSUCKER( ch ) && IS_OUTSIDE( ch ) )
  {
...
ch_ret multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( (retcode = one_hit( ch, victim, dt )) != rNONE && retcode != rSPELL_FAILED )
      return retcode;
...
int weapon_prof_bonus_check( CHAR_DATA *ch, OBJ_DATA *wield, int *gsn_ptr )
{
	...
	if ( *gsn_ptr != -1 )
	  bonus = (int) ((LEARNED(ch, *gsn_ptr) -50)/10);
       ...
       {
          bonus = bonus * 1000 / 
          ( 1 + abs( ch->alignment - ch->pcdata->faction->alignment ) );
       }*/
...
ch_ret one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( (retcode = damage( ch, victim, dam, dt )) != rNONE && retcode != rSPELL_FAILED )
	return retcode;
    if ( char_died(ch) )
	return rCHAR_DIED;
    if ( char_died(victim) )
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	else if ( ch->fighting->who == victim )
        {
	    if ( ((ch->fighting->xp * dam)/victim->max_hit) > 0 )
	    {
		xp_gain = ( ch->fighting->xp * dam ) / victim->max_hit;
	    }
	    else if ( ch->fighting->xp > victim->max_hit && ((ch->fighting->xp / victim->max_hit)*dam) > 0 )
	    {
		xp_gain = ( ch->fighting->xp/victim->max_hit ) * dam;
	    }
	    else if ( dam > victim->max_hit && (ch->fighting->xp * (dam/victim->max_hit)) > 0 )
	    {
		xp_gain = ( ch->fighting->xp * (dam/victim->max_hit) );
	    }
	    //This is a last resort, in the case that there is no easy way to get the xp_gain value, courtesy of Aurora
	    else
	    {
		int MAX_XP_GAIN = URANGE( (ch->level * 10), (int) (ch->level * ch->level * 0.85), 56765 );
		xp_gain = (victim->max_hit/ch->level) > 0 ? (victim->max_hit/ch->level) : MAX_XP_GAIN;
	    }
	}
	gain_exp( ch, xp_gain );
    }
	...
	if ( !npcvict )
	{
	    ...
	    to_channel( log_buf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL );
            ...
            if (IS_CLANNED(ch) && !IS_IMMORTAL( ch ) && victim != ch )
            {
		  ...
		  !CAN_PKILL( victim ) ? "&W<Peaceful>" :
		    victim->pcdata->faction ? victim->pcdata->faction->badge :
		      "&P(&WUnclanned&P)",
		  victim->name,
                  ch->in_room->area->name );
		if ( victim->pcdata->faction &&
		     victim->pcdata->faction->name == ch->pcdata->faction->name)
		;
		else
...
bool legal_loot( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_NPC(ch) && !ch->master )
      return TRUE;
    /* members of different factions can loot too! -Thoric */
    if ( !IS_NPC(ch) && !IS_NPC(victim)
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
	  int level_ratio;
	  level_ratio = URANGE( 1, ch->level / victim->level, LEVEL_DEMIGOD);
	  if ( ch->pcdata->faction )
	    ch->pcdata->faction->mkills++;
	  ch->pcdata->mkills++;
	...
	if ( !IS_NPC( ch ) )
	{
	  if ( ch->pcdata->faction )
	  {
	    if ( victim->level < ((LEVEL_DEMIGOD / 10) ) )
	      ch->pcdata->faction->pkills[0]++;
	    else if ( victim->level < (LEVEL_DEMIGOD / 5) )
	      ch->pcdata->faction->pkills[1]++;
	    else if ( victim->level < ((LEVEL_DEMIGOD / 5) * 2) )
	      ch->pcdata->faction->pkills[2]++;
	    else if ( victim->level < ((LEVEL_DEMIGOD / 5) * 3) )
	      ch->pcdata->faction->pkills[3]++;
	    else if ( victim->level < ((LEVEL_DEMIGOD / 5) * 4) )
	      ch->pcdata->faction->pkills[4]++;
	    else if ( victim->level < LEVEL_DEMIGOD )
	      ch->pcdata->faction->pkills[5]++;
	    else
	      ch->pcdata->faction->pkills[6]++;
	  }	
	  ch->pcdata->pkills++;
    ...
    {
	if ( IS_CLANNED( ch ) && ( !IS_CLANNED( victim ) || ch->pcdata->faction != victim->pcdata->faction ) ) 
	{
	    if ( victim->level < (LEVEL_DEMIGOD / 10) )
		ch->pcdata->faction->pkills[0]++;
	    else if ( victim->level < (LEVEL_DEMIGOD / 5) )
	...
	if ( IS_CLANNED( victim ) && ( !IS_CLANNED( ch ) || ch->pcdata->faction != victim->pcdata->faction ) ) 
	{
            if ( ch->level < (LEVEL_DEMIGOD / 10) )
              victim->pcdata->faction->pdeaths[0]++;
            else if ( ch->level < ((LEVEL_DEMIGOD / 5) * 1) )
              victim->pcdata->faction->pdeaths[1]++;
            else if ( ch->level < ((LEVEL_DEMIGOD / 5) * 2) )
              victim->pcdata->faction->pdeaths[2]++;
            else if ( ch->level < ((LEVEL_DEMIGOD / 5) * 3) )
              victim->pcdata->faction->pdeaths[3]++;
            else if ( ch->level < ((LEVEL_DEMIGOD / 5) * 4) )
              victim->pcdata->faction->pdeaths[4]++;
            else if ( ch->level < LEVEL_DEMIGOD )
              victim->pcdata->faction->pdeaths[5]++;
            else
              victim->pcdata->faction->pdeaths[6]++;
	}
	victim->pcdata->pdeaths++;   
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( IS_QUESTING( ch ) && IS_NPC( victim ) )
	{
		...
		{
		    send_to_char( "You have annihilated your target!\n\r", ch );
		    ch_printf( ch, "Return to %s before it's too late!\n\r", NAME( ch->pcdata->questmaster ) );
		    ch->pcdata->quest_type = QT_COMPLETE;
		}
	    }
	    else if ( ch->pcdata->quest_type == QT_MOB_GIVE || ch->pcdata->quest_type == QT_MOB_GIVE_COMPLETE )
	    {
		if ( (ch->pcdata->quest_mob) == (victim->pIndexData->vnum) )
		{
		    ch->pcdata->quest_type = QT_FAILED;
		    send_to_char( "You have annihilated your target... BUT YOU WEREN'T SUPPOSED TO!\n\r", ch );
		    ch_printf( ch, "Return to %s to report your failure!\n\r", NAME( ch->pcdata->questmaster ) );
		}
	    }
	}
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC( ch ) )
	{
	    ...
	    {
	      int level_ratio = URANGE( 1, wf->level / ch->level, LEVEL_DEMIGOD );
    ...
    if ( ch->level < LEVEL_DEMIGOD && number_bits( 3 ) == 1 )
    {
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
	...
	corpse->value[1] = (int)ch->max_hit; */ /* Using corpse cost to cheat, since corpses not sellable */
