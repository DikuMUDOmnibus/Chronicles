...
"nolocate", "groundrot", "lootable", "pkdisarmed" };
...
{
"antipkill", "freekill", "noteleport", "spelllimit", "noquest", "nopkill", "r6", "r7", 
...
"salve", "cook", "keyring", "odor", "chance", "slotmachine"
};
...
"hold",			"sanctuary",		"faerie_fire",
"infrared",		"curse",		"_flaming",
"poison",		"protect",		"_paralysis",
"sneak",		"hide",			"sleep",
"charm",		"flying",		"pass_door",
"floating",		"truesight",		"detect_traps",
"scrying",		"fireshield",		"shockshield",
"detect_sneak",		"iceshield",		"possess", 
...
{
"npc",		"sentinel",	"scavenger",	"questmaster",	"noquest",
"aggressive",	"stayarea",	"wimpy",	"pet",		"banker",
"practice",	"immortal",	"deadly",	"polyself",	"meta_aggr",
...
"nstart",	"dnd",		"idle",		"showslots",	"admin",
"enforcer",	"builder",	"roleplayer",	"r17",		"r18",
"r19",		"r20",		"r21",		"r22",		"r23",
"r24",		"r25"
};
...
"deny", "freeze", "thief","killer", "litterbug", "ansi", "emptyspace", "nice", 
"flee" ,"autogold", "unused", "afk", "invisprompt", "autoglance", "autonews",
"questing"
};
...
"builder",	"roleplayer",	"wholename",	"r7",		"r8", 
"r9",		"r10",		"r11",		"r12",		"r13",
"r14",		"r15",		"r16",		"r17",		"r18",
"r19",		"r20",		"r21",		"r22",		"r23",
"r24",		"r25",		"r26",		"r27",		"r28",
...
"hands",		"feet",			"fingers",
"ear",			"eye",			"long_tongue",
"eyestalks",		"tentacles",		"fins",
"wings",		"tail",			"scales",
"claws",		"fangs",		"horns",
"tusks",		"tailattack",		"sharpscales",
"beak",			"haunches",		"hooves",
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    /*
     * Wraparound to allow easier movement for builders. -Orion
     */
    if ( !str_cmp( arg, "next" ) || !str_cmp( arg, "prev" ) )
    {
	int nextRoom = 0;
	/*
	 * If the vnum you are going to is over the size of temp, something has gone
	 * SEVERELY wrong. Used 15 to give sufficient space in the array. A value of
	 * 13 should work fine, but I figured why not. -Orion
	 */
	char temp[15];

	/*
	 * We COULD handle this here. But that could cause problems if they aren't
	 * SUPPOSED to be in a room at the moment (don't ask me how... I'm being
	 * lazy, because I've been working on this for several hours now... go
	 * bother someone else :P). -Orion
	 */
	...
	if ( get_trust( ch ) < LEVEL_WORKER || vnum < 1 || vnum > MAX_VNUM || IS_NPC(ch) || 
	     !ch->pcdata->area )
	{
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  name short long description title spec faction\n\r", ch );
    ...
    if ( !victim && get_trust( ch ) < LEVEL_WORKER )
    {
	...
	{
	    send_to_char( "Hp range is 1 to 32,767 hit points.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Mana range is 0 to 32,767 mana points.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 32767 )
	{
	    send_to_char( "Move range is 0 to 32,767 move points.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Blood application range is 0 to 2,147,000,000 blood points.\n\r", ch );
	    return;
	}
	...
	if ( !arg3 || arg3[0] == '\0' )
	{
	    ...
	    if ( !IS_IMMORTAL( victim ) ) {
	        ...
	        save_faction( victim->pcdata->faction );
	    }
	...
	faction = get_faction( arg3 );
	if ( !faction )
	{
	   send_to_char( "No such faction.\n\r", ch );
	   return;
	}
	if ( victim->pcdata->faction != NULL && !IS_IMMORTAL( victim ) )
	{
		--victim->pcdata->faction->members; 
	        save_faction( victim->pcdata->faction );
        }
    ...
    {
	if ( !IS_NPC( victim ) && get_trust( ch ) < LEVEL_WORKER )
	{
	    send_to_char( "You can only modify a mobile's flags.\n\r", ch );
	    return;
	}
	...
	if ( victim->pcdata->faction && !IS_IMMORTAL(victim))
	{
...
void do_oset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !obj && get_trust(ch) < LEVEL_WORKER )
    {
	...
	if ( obj->item_type == ITEM_PAPER && get_trust(ch) < LEVEL_MASTER )
	{
...
void fold_area( AREA_DATA *tarea, char *filename, bool install )
{
    ...
    log_string_plus( buf, LOG_NORMAL, LEVEL_MASTER );
    ...
    if ( tarea->room_name_format )
	fprintf( fpout, "#NAMEFORMAT %s~\n\n", tarea->room_name_format );
    if ( tarea->room_desc_format )
	fprintf( fpout, "#DESCFORMAT %s~\n\n", tarea->room_desc_format );
...
void do_astat( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "\n\r&wName:     &W%s\n\r&wFilename: &W%-20s  &wPrototype: &W%s\n\r&wAuthor:   &W%s\n\r",
			...
			proto ? "yes" : "no",
			tarea->author );
    if ( tarea->room_name_format && str_cmp( tarea->room_name_format, "" ) )
    {
	send_to_char_color( "&wName_format: &W", ch );
	ch_printf      ( ch, "%s\n\r", tarea->room_name_format );
	ch_printf_color( ch, "    &wExample: &W%s\n\r", tarea->room_name_format );
    }
    if ( tarea->room_desc_format && str_cmp( tarea->room_desc_format, "" ) )
    {
	send_to_char_color( "&wDesc_format: &W", ch );
	ch_printf      ( ch, "%s\n\r", tarea->room_desc_format );
	ch_printf_color( ch, "    &wExample: &W%s(TEST TEXT)\n\r", tarea->room_desc_format );
    }
...
void do_aset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  author derivatives resetmsg resetfreq flags\n\r", ch );
	return;
    }
    ...
    {
	bool fDeriv = TRUE;
	char new_deriv[MAX_STRING_LENGTH];
	...
	sprintf( new_deriv, "%s%s%s", fDeriv ? tarea->derivatives : "", fDeriv ? " " : "", argument );
    ...
    if ( !str_cmp( arg2, "descformat" ) )
    {
	if ( tarea->room_desc_format )
	    STRFREE( tarea->room_desc_format );
	tarea->room_desc_format = STRALLOC( argument );
	send_to_char( "Done.\n\r", ch );
	return;
    }

    if ( !str_cmp( arg2, "nameformat" ) )
    {
	if ( tarea->room_name_format )
	    STRFREE( tarea->room_name_format );
	tarea->room_name_format = STRALLOC( argument );
	send_to_char( "Done.\n\r", ch );
	return;
    }
...
void do_rlist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) || get_trust( ch ) < LEVEL_SLAVE || !ch->pcdata
    || ( !ch->pcdata->area && get_trust( ch ) < LEVEL_MASTER ) )
    {
