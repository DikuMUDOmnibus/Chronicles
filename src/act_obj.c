...
void	save_faction_storeroom	args( ( CHAR_DATA *ch, FACTION_DATA *faction ) );
...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    if ( xIS_SET( obj->extra_flags, ITEM_PKDISARMED )
    &&  !IS_NPC( ch ) )
    {
...
/*	if (!char_died) save_char_obj(ch); */
	for ( faction = first_faction; faction; faction = faction->next )
	  if ( faction->storeroom == ch->in_room->vnum )
	    save_faction_storeroom(ch, faction);
    }
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_QUESTING( ch ) )
    {
	if ( IS_NPC( victim ) && ch->pcdata->quest_type == QT_MOB_GIVE )
	{
		...
		sprintf( quest_buf, "0.%s Thank you, thank you for bringing me %s. I really appreciate it.", ch->name, obj->short_descr );
		do_whisper( victim, quest_buf );
		sprintf( quest_buf, "0.%s I'll send a messenger to %s now, alerting him that you were here.", ch->name, NAME( ch->pcdata->questmaster ) );
		do_whisper( victim, quest_buf );
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( !IS_NPC( ch ) && ch->pcdata->faction && ch->pcdata->faction->deity[0] != '\0' )
    {
	strcpy( name, ch->pcdata->faction->deity );
    }
    else
    {
	strcpy( name, "Thoric" );
...
/*
 * Save items in a faction storage room			-Scryn & Thoric
 */
void save_faction_storeroom( CHAR_DATA *ch, FACTION_DATA *faction )
{
    FILE *fp;
    char filename[256];
    sh_int templvl;
    ...
    if ( !faction )
    {
	bug( "save_faction_storeroom: Null faction pointer!", 0 );
	return;
    }

    if ( !ch )
    {
	bug ("save_faction_storeroom: Null ch pointer!", 0);
	return;
    }
    ...
    {
	bug( "save_faction_storeroom: fopen", 0 );
	perror( filename );
    }
    else
    {
...
/*
 * Advanced ascii atoi function (supports special characters). -Orion
 */
int advatoi (char *s)
{
