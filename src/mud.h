...
/*
 * VNUM definition to make changing VNUMs around easier. -Orion
 */
typedef	sh_int				VNUM;
...
#define MAX_VNUM		32000	/* Maximum room, object, and mobile vnum */
...
#define LEVEL_HIGH_ADMIN	(MAX_LEVEL - 1)
#define LEVEL_MASTER		(MAX_LEVEL - 2)
#define LEVEL_WORKER		(MAX_LEVEL - 3)
#define LEVEL_SLAVE		(MAX_LEVEL - 4)

#define LEVEL_IMMORTAL		(MAX_LEVEL - 4)

#define LEVEL_DEMIGOD		(MAX_LEVEL - 5)
#define LEVEL_AVATAR		(((LEVEL_DEMIGOD / 4) * 3) + 1)
#define LEVEL_HERO		 ((LEVEL_DEMIGOD / 4) * 3)

#define LEVEL_LOG		LEVEL_SLAVE
#define LEVEL_HIGOD		LEVEL_MASTER
...
#define CODEBASE_VERSION_MINOR	"20a"
#define CODEBASE_VERSION_INFO	"    Chronicles is a new codebase that combines many of the more sought out features with the general Diku based style. Chronicles, along with a few other select codebases, tries to provide usability, portability, and power without sacrifice memory and disk space.\n\r    This is the second non-beta release of Chronicles and hopefully will be as stable as the first non-beta release. As always, should you have problems, please notify us at http://forums.mudplanet.org, in the Chronicles section. All bug fixes to this codebase will be posted there.\n\r    I have worked hard to provide this base publically. Please, follow the license and make me glad that I've done so. :)\n\r\n\r-Orion Elder"
...
#define XBI		2	/* integers in an extended bitvector	*/
...
typedef enum
{
  CON_PLAYING = 0,	CON_GET_NAME = -99,	CON_GET_OLD_PASSWORD,
  ...
  CON_GET_WANT_ANSI,	CON_TITLE,		CON_PRESS_ENTER,
  CON_WAIT_1,		CON_WAIT_2,		CON_WAIT_3,
  CON_ACCEPTED,         CON_GET_PKILL,		CON_READ_IMOTD,
  CON_PRESS_ENTER_MENU,	CON_MENU,		CON_READ_NEWS,
...
/* Races */
typedef enum {
  RACE_HUMAN,		RACE_ELF,		RACE_DWARF,
  RACE_HALFLING,	RACE_PIXIE,		RACE_HALF_OGRE,
  RACE_HALF_ORC,	RACE_HALF_TROLL,	RACE_HALF_ELF,
  RACE_GITH,		RACE_DROW,		RACE_SEA_ELF,
  RACE_LIZARDMAN,
  RACE_DRAGON = 31
} race_types;

typedef enum {
  CLASS_NONE = -1,	CLASS_MAGE = 0,		CLASS_CLERIC,
  CLASS_THIEF,		CLASS_WARRIOR,		CLASS_VAMPIRE,
  CLASS_DRUID,		CLASS_RANGER,		CLASS_AUGURER,
  CLASS_PALADIN,
} class_types;
...
typedef enum
{
  ...
  ACT_QUESTMASTER,	ACT_NOQUEST,		ACT_AGGRESSIVE,
  ACT_STAY_AREA,	ACT_WIMPY,		ACT_PET,
  ACT_BANKER,		ACT_PRACTICE,		ACT_IMMORTAL,
  ACT_DEADLY,		ACT_POLYSELF,		ACT_META_AGGR,
...
typedef enum
{
  ...
  AFF_HOLD,		AFF_SANCTUARY,		AFF_FAERIE_FIRE,
  AFF_INFRARED,		AFF_CURSE,		AFF_FLAMING,
...
typedef enum
{
  ...
  SF_REVERSE,		SF_NOSELF,		SF_UNUSED2,
...
typedef enum
{
  ...
  ITEM_EMPTY1,		ITEM_EMPTY2,		ITEM_MISSILE_WEAPON,
  ITEM_PROJECTILE,	ITEM_QUIVER,		ITEM_SHOVEL,
  ITEM_SALVE,		ITEM_COOK,		ITEM_KEYRING,
  ITEM_ODOR,		ITEM_CHANCE,		ITEM_SLOT_MACHINE,
  MAX_ITEM_TYPE
} item_types;
...
typedef enum
{
  ...
  ITEM_ANTI_THIEF,	ITEM_ANTI_WARRIOR,	ITEM_ANTI_CLERIC,
  ...
  ITEM_COVERING,	ITEM_DEATHROT,		ITEM_BURIED,
  ITEM_PROTOTYPE,	ITEM_NOLOCATE,		ITEM_GROUNDROT,
  ITEM_LOOTABLE,	ITEM_PKDISARMED, 
  MAX_ITEM_FLAG
} item_extra_flags;
...
typedef enum
{
  ...
  APPLY_WEAPONSPELL,	APPLY_LCK,		APPLY_BACKSTAB,
  APPLY_PICK,		APPLY_TRACK,		APPLY_STEAL,
  APPLY_SNEAK,		APPLY_HIDE,		APPLY_PALM,
  APPLY_DETRAP,		APPLY_DODGE,		APPLY_PEEK,
  APPLY_SCAN,		APPLY_GOUGE,		APPLY_SEARCH,
  APPLY_MOUNT,		APPLY_DISARM,		APPLY_KICK,
  APPLY_PARRY,		APPLY_BASH,		APPLY_STUN,
  APPLY_PUNCH,		APPLY_CLIMB,		APPLY_GRIP,
  APPLY_SCRIBE,		APPLY_BREW,		APPLY_WEARSPELL,
  APPLY_REMOVESPELL,	APPLY_EMOTION,		APPLY_MENTALSTATE,
  APPLY_STRIPSN,	APPLY_REMOVE,		APPLY_DIG,
  APPLY_FULL,		APPLY_THIRST,		APPLY_DRUNK,
  APPLY_BLOOD,		APPLY_COOK,		APPLY_RECURRINGSPELL,
...
typedef enum
{
  ...
  PLR_AUTOSAC,	PLR_BLANK,	PLR_OUTCAST,	PLR_BRIEF,	PLR_COMBINE,
  ...
  PLR_ANSI,	PLR_EMPTYSPACE,	PLR_NICE,	PLR_FLEE,	PLR_AUTOGOLD,
  PLR_UNUSED,	PLR_AFK,	PLR_INVISPROMPT,PLR_AUTOGLANCE,	PLR_AUTONEWS,
  PLR_QUESTING
} player_flags;
...
#define PCFLAG_ROLEPLAYER	   BV22 /* Set an Immortal up as an Roleplayer	*/
...
struct	char_data
{
    ...
    sh_int		move;
    sh_int		max_move;
    sh_int		practice; /* PC DATA ? */
    sh_int		numattacks;
    int			gold;
    int			exp; /* Should be in PC_DATA, but I plan on mobs needing soon -Orion */
    EXT_BV		act; /* NPC */
    EXT_BV		affected_by; 
    ...
    sh_int		mod_cha;
    sh_int		mod_lck;
    sh_int		mental_state;		/* simplified */ /* PC DATA ? */
    sh_int		emotional_state;	/* simplified */ /* PC DATA ? */
    int			retran;
    int			regoto;
    sh_int		mobinvis;	/* Mobinvis level SB */ /* NPC */
...
struct	pc_data
{
    ...
    CHAR_DATA *		questmaster;		 /* Orion Elder */
    int			quest_timer[2];          /* Orion Elder */
    int			quest_type;		 /* Orion Elder */
    int			quest_mob;		 /* Orion Elder */
    OBJ_DATA  *		quest_obj;		 /* Orion Elder */
    AREA_DATA *		quest_area;		 /* Orion Elder */
};
...
struct	area_data
{
    ...
    char *		room_name_format; /* Orion */
    char *		room_desc_format; /* Orion */
...
struct	system_data
{
    ...
    sh_int	think_level;		/* Level of think channel LEVEL_MASTER*/
    sh_int	build_level;		/* Level of build channel LEVEL_WORKER*/
    sh_int	log_level;		/* Level of log channel LEVEL LOG*/
    sh_int	level_modify_proto;	/* Level to modify prototype stuff LEVEL_WORKER */
    ...
    sh_int	reqwho_arg;		/* Who argument required? */
    sh_int	wiz_lock;		/* Game wizlocked? */
    sh_int	newbie_stats[3];	/* Newbie starting stats */
    sh_int	autoboot_hour;		/* What hour does the autoboot happen? */
    sh_int	autoboot_minute;	/* What minute does the autoboot happen? */
    sh_int	autoboot_period;	/* What period of days pass between autoboots? */
};
 ...
 * Character macros.
 */
#define NAME(ch)                ((ch) == NULL ? "" : IS_NPC((ch)) ? (ch)->short_descr : (ch)->name)
#define IS_NPC(ch)		(xIS_SET((ch)->act, ACT_IS_NPC))
#define IS_IMMORTAL(ch)		(get_trust((ch)) >= LEVEL_IMMORTAL)
#define IS_DEMIGOD(ch)		(get_trust((ch)) >= LEVEL_DEMIGOD)
#define IS_AVATAR(ch)		(get_trust((ch)) >= LEVEL_AVATAR)
...
#define CAN_CAST(ch)		((ch)->class != CLASS_THIEF && (ch)->class != CLASS_WARRIOR)

#define IS_BLOODSUCKER(ch)	(!IS_NPC(ch)				    \
...
#define WAIT_STATE(ch, npulse)	( IS_IMMORTAL( ch ) ? (ch)->wait = 0 : WAIT_EVAL( ch, npulse ) )
...
#define CMD_WHOLE_NAME		BV07	/* Require the whole command name	*/ 
...
void	teleport	args( ( CHAR_DATA *ch, sh_int room, EXT_BV *flags ) );
...
bool fptof	args( ( FILE *stream, char *data ) );
