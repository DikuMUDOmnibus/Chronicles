...
do_morph_char ( CHAR_DATA *ch, MORPH_DATA *morph )
{
  ...
  if ( morph->manaused && !IS_BLOODSUCKER(ch) )
  {
  ...
  if ( morph->bloodused && IS_BLOODSUCKER(ch) ) 
  {
...
do_morph ( CHAR_DATA *ch, MORPH_DATA *morph )
{
  ...
  else
  {
  	...
  	if ( (ch->mana + ch_morph->mana ) > 32767 )
