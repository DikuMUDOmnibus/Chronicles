...
sh_int get_trust( CHAR_DATA *ch )
{
	...
	return LEVEL_DEMIGOD;

    if ( ch->level >= LEVEL_SLAVE && IS_RETIRED( ch ) )
      return LEVEL_SLAVE;
...
void obj_from_char( OBJ_DATA *obj )
{
    ...
    ch->carry_weight	-= is_magic_container( obj ) ? obj->weight : get_obj_weight(obj);
    return;
}
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust(ch) >= LEVEL_SLAVE && is_number( arg ) )
	vnum = atoi( arg );
    else
	vnum = -1;
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
	...
	if ( ch->desc )
	{
	    DESCRIPTOR_DATA *ch_d = ch->desc;

	    /*
	     * Menu who fix. If they're in those states their room information is
	     * more than likely going to be NULL. -Orion
	     */
	    if ( ch_d->connected == CON_MENU || ch_d->connected == CON_PRESS_ENTER_MENU )
		hasRoom = FALSE;
	}
	...
	if ( hasRoom && room_is_dark( ch->in_room ) && !IS_AFFECTED(ch, AFF_INFRARED) )
	  return FALSE;
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    if ( xIS_SET(*vector, AFF_DETECT_SNEAK )) strcat( buf, " detect_sneaking");
